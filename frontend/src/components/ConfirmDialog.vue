...
const { t } = useI18n()
const { current, confirm, cancel } = useConfirmDialog()
  ...
  <transition name="fade">
    <div v-if="visible" class="overlay" @click="cancel">
      <div class="dialog" @click.stop>
        ...
        <div class="footer">
          <button class="btn cancel-btn" @click="cancel">{{ t('confirm.cancel') }}</button>
          <button class="btn danger-btn" @click="confirm">{{ t('confirm.ok') }}</button>
        </div>
      </div>
    </div>
  </transition>
