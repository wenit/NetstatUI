...
- `services/geo/` — IP → 国家-城市 解析（ip2region 离线库）。`Resolver.Lookup(addr)`；`VIndexCache` + searcher 池（每文件 4 个 searcher，总 8 个 fd）；自带 LRU 4096；跳过非公网 IP。**懒加载**：`New()` 不做 I/O；`InitAsync(onReady)` 后台 extract + init；`Lookup` 在 init 完成前返回 ""。`materialize()` 复用 `<exe-dir>/data/` side-load 优先；缓存路径 `os.UserCacheDir()/NetstatUI/data/`。
...
- **地理位置**：`ConnInfo.Geo` 字段是 `monitor.collect()` 调用 `geo.Resolver.Lookup` 填充的 `"国家-城市"` 字符串；非公网 / 监听 / 无效 IP 一律为空。`xdb` 文件首次启动从 `embed.FS` 解压到 `os.UserCacheDir()/NetstatUI/data/`（`<exe-dir>/data/` 内手放可 side-load 优先）。`geo.Resolver` 是懒加载：`main()` 里的 `geo.New()` 瞬时返回，`Monitor.ServiceStartup` 调 `InitAsync(refreshAfterGeoReady)` 在后台 goroutine 初始化 searcher 池；init 完成后回调里 `RefreshNow()` 重发 `conn:full` 批量回填 Geo。前端订阅 `geo:status` 事件 + `useGeoStatus` composable 维护 `loading`/`ready`/`error` 状态；Geo 列在 loading 期间显示 `…`。前端开关 `np.geo` 只控 UI 显隐，后端始终尝试解析。
