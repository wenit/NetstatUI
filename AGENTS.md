...
## 必读入口
- `main.go` — Wails 3 application 入口：注册服务、创建 Frameless + Mica 窗口、注册事件类型（`conn:full/conn:diff/conn:stats/conn:error/geo:status`）。`geoDataDir()` 返回 `<exe-dir>/data/`（ip2region xdb 旁加载，不再 embed）。
...
- `services/geo/` — IP → 国家-城市 解析（ip2region 离线库）。`Resolver.Lookup(addr)`；`VIndexCache` + searcher 池（每文件 4 个 searcher，总 8 个 fd）；自带 LRU 4096；跳过非公网 IP。**懒加载**：`New(dataDir)` 不做 I/O；`InitAsync(onReady)` 后台验证 `<exe-dir>/data/ip2region_{v4,v6}.xdb` 并 init searcher；`Lookup` 在 init 完成前返回 ""。若数据文件缺失，`geo:status` 进入 `error`，应用继续运行。
- `services/kill/` — `windows.TerminateProcess` 结束进程（仅 `PROCESS_TERMINATE` 权限）。
- `services/system/` — `GetSystemLocale()` 返回 OS locale 字符串。
- `data/` — ip2region xdb 文件（`ip2region_v4.xdb` + `ip2region_v6.xdb`），通过 `scripts/download-xdb.ps1` 拉取；随 release 包与二进制同级分发，开发时保留在仓库中。
...
- **地理位置**：`ConnInfo.Geo` 字段是 `monitor.collect()` 调用 `geo.Resolver.Lookup` 填充的 `"国家-城市"` 字符串；非公网 / 监听 / 无效 IP 一律为空。`xdb` 文件位于 `<exe-dir>/data/`（随 release 包分发，不再 embed）。`geo.Resolver` 是懒加载：`main()` 里的 `geo.New(dataDir)` 瞬时返回，`Monitor.ServiceStartup` 调 `InitAsync(refreshAfterGeoReady)` 在后台 goroutine 初始化 searcher 池；文件缺失时捕获错误、`geo:status = error`、应用继续运行；init 完成后回调里 `RefreshNow()` 重发 `conn:full` 批量回填 Geo。前端订阅 `geo:status` 事件 + `useGeoStatus` composable 维护 `loading`/`ready`/`error` 状态；Geo 列在 loading 期间显示 `…`。前端开关 `np.geo` 只控 UI 显隐，后端始终尝试解析。
...
- **更新 xdb 离线库**：跑 `pwsh scripts/download-xdb.ps1`（或手动下载到 `data/`），然后 `wails3 build` / `build.ps1`。`build.ps1` 与 CI 会把 `data/` 自动复制到 `bin/data/` 或 release 包内，保证二进制旁加载可用。
