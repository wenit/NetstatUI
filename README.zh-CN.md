...
> Win11 Fluent 风格的桌面网络端口 / 连接查看器（类 `netstat`）。

[English](./README.md) · [简体中文](./README.zh-CN.md)

一个**只读**的 Windows 桌面工具，列出本机所有 TCP/UDP 连接，并显示对应的 PID、进程名与可执行文件路径 —— 一键结束进程。基于 **Wails 3** + **Vue 3**，自带 Fluent **Mica** 毛玻璃无边框窗口。
...
- 🎨 **Win11 Fluent UI** — Mica 毛玻璃背景、明 / 暗 / 自动三主题、自绘标题栏、紧凑 / 舒适两种密度。
- ⚡ **实时增量刷新** — diff 推流；首帧 `conn:full`，后续只推 `added` / `removed` / `updated`。
- 🚀 **自写虚拟滚动** — 万行级数据 60 fps 流畅渲染（绝对定位 + transform）。
- 🔍 **强大筛选** — 全字段搜索、协议 chip、状态 chip、仅 LISTEN / 仅外网切换。
- 🪓 **一键结束进程** — 确认弹框 → `TerminateProcess`，结束后立即自动刷新。
- 🌍 **中英双语** — 默认英文 / 简体中文，OS locale 自动识别。
...
# 生产构建
.\build.ps1                  # Windows（推荐，绕过文件锁问题）
# 或
wails3 build                 # macOS / Linux
```

产物路径：`bin/NetstatUI.exe`（Windows）/ `bin/NetstatUI`（Unix）。
...
- **部分 loopback listener 可能缺失** —— Windows 11 22H2+ 的 `GetTcpTable2` / `GetExtendedUdpTable` 会静默丢弃一部分 `127.0.0.1` LISTEN 连接。`netstat -ano` 用的是 WMI 路径所以能看到。本工具与 `go-netstat` 走相同路径，存在同样限制。
- **仅 Windows** —— Linux / macOS 的 provider 在 `services/netstat/provider.go` 接口里有预留，但暂未实现。UI 与 `kill` 服务同样为 Windows 专属。
